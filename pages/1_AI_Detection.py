...
# ── 1. LOAD MODEL (สถาปัตยกรรมการโหลดโมเดลแบบ Caching เพื่อความปลอดภัย) ──
@st.cache_resource
def load_ovasight_model():
    # ตรวจสอบพิกัดของไฟล์ภายในโปรเจกต์โลคอลของคุณ
    ...
    try:
        # โหลดโมเดลตัวจริงของคุณผ่าน Custom Object
        return load_model(model_path, custom_objects={'mse': mse})
    except Exception as e:
        # แสดงกล่องแจ้งเตือนแนวทางแก้ไขทางหน้าจอหลักหากเลเยอร์ Python สัญญาณขาดหาย
        st.error(f"⚠️ ตรวจพบข้อผิดพลาดขณะดึงโครงสร้างโมเดล Keras: {e}")
        st.info("💡 แนะนำสำหรับเครื่อง Windows: เปิด Anaconda Prompt แล้วพิมพ์: `pip install tensorflow==2.15.0` เพื่อตั้งค่าตัวแปลภาษาให้เข้าคู่กับไฟล์โมเดลค่ะ")
        return None

model = load_ovasight_model()
class_label = ["Artifact", "Ev eggs"]

# ── 2. CORE ALGORITHM FUNCTIONS (ฟังก์ชันดั้งเดิมของคุณทั้งหมด) ──────────────────
def drawbox(img, label, a, b, c, d, color):
    text_color = (6, 64, 43)
    # ปรับลดระดับ Font Scale และความหนาของเส้นกล่องให้อ่านง่ายขึ้นบนหน้าจอ Web UI
...
# ── 3. INTERACTIVE CONTROL PANEL (แถบปรับแต่งสำหรับทดลองและวินิจฉัย) ──
...
if uploaded_file is not None:
    # อ่านไฟล์ไบนารีจาก Streamlit Uploader สู่รูปแบบ OpenCV Image Matrix
    ...
    if model is not None:
        with st.spinner('⏳ ระบบกำลังประมวลผลอัลกอริทึม Sliding Window และตรวจวิเคราะห์พยาธิวิทยา...'):
            ...
            # ── 4.1 กระบวนการคัดกรองแถบภาพย่อย (Sliding Window Stage) ──
            ...
            if len(patches) > 0:
                ...
                # ── 4.2 คัดแยกพิกัดคลาสตามเงื่อนไข Thresh ──
                ...
                # ── 4.3 คำนวณขจัดกล่องซ้ำและผสานกล่อง (NMS & Merge Box) ──
                ...
                # ── 4.4 การเรนเดอร์และการวาดผลลัพธ์ลงบนวัตถุภาพ ──
                img_output = img.copy()
                ...
                # ── 5. PROFESSIONAL RESULTS SCREEN (แสดงผลแบ่งสองฝั่งแบบหน้าต่างแล็บสากล) ──
                ...
                with col_out2:
                    ...
                    # แปลงโครงสร้างสี BGR เป็น RGB ก่อนส่งให้สตรีมลิตเรนเดอร์ภาพขึ้นหน้าจอเว็บ
